    ...
    if ($acao == 'excluir') {
        ...
        if ($delataVeiculo->rowCount() == 0) {
            ...
            echo "<script>alert('Registro excluido com sucesso!');
                    location.href=\"index.php\"</script>";
            auditoria("Veiculo id ".$id." deletado" );
        }
    }
...
if (isset($_POST['cd_veiculo']) && $_POST['cd_veiculo'] != '') {
    ...
    if (empty($id)) {
        ...
        try {
            ...
            //  exit();
            auditoria("Veiculo id ".$conexao->lastInsertId()." Inserido" );
            $retorno = 'inserido';
        } catch (Exception $e) {
            echo $e;
            exit($e);
        ...
        try {
            ...
            $atualizarMulta = $conexao->prepare("UPDATE multa SET cd_modalidade = :modalidade, cd_prefixo = :prefixo " . "WHERE cd_placa = :placa");
            $atualizarProcesso = $conexao->prepare("UPDATE processo SET cd_modalidade = :modalidade " . "WHERE cd_placa = :placa");
            ...
            $atualizarMulta->bindValue(":modalidade", $modalidade);
            $atualizarProcesso->bindValue(":modalidade", $modalidade);
            ...
            $atualizarMulta->bindValue(":placa", $placa);
            $atualizarProcesso->bindValue(":placa", $placa);
            ...
            //
            // echo $atualizarProcesso->rowCount();
            // var_dump($atualizarProcesso);
            // echo $atualizarProcesso->errorCode();
            //exit();
            auditoria("Dados do veiculo id ".$id." atualizados" );
            $retorno = 'alterado';
        } catch (Exception $e) {
            echo $e;
            exit();
