...
<?php if ($veiculos->rowCount() > 0) { ?>
 ...
 <?php
        ...
        <!--                                                                    <a onClick="buscaUsuario('<?php echo($veiculo['cd_veiculo']); ?>');location.href='gerencia.php?acao=visualizar&id_usuario=<?php echo $veiculo['cd_veiculo']; ?>'" class="btn btn-success"><i class="icon-trash">&nbsp;&nbsp;Visualizar</i> </a>-->
                                                                    ...
                                                                    <button type="button" data-toggle="modal" data-target="#myModal<?php echo $veiculo['cd_placa']; ?>"  class="btn btn-inverse" ><i class="icon-eye-open">&nbsp;&nbsp;Ver Multas</i></button>
                                                                    <button type="button" data-toggle="modal" data-target="#myModalProcesso<?php echo $veiculo['cd_placa']; ?>"  class="btn btn-inverse" ><i class="icon-eye-open">&nbsp;&nbsp;Ver Processos</i></button>

                                                                </td>
                                                            </tr>
        <?php
        $cd_placaMulta = $veiculo['cd_placa'];

        try {
            $multas = $conexao->prepare("SELECT * FROM multa where cd_placa = :cd_placa");

            $multas->bindValue(":cd_placa", $cd_placaMulta, PDO::PARAM_STR);
        ...
        ?>
                                                        ...
                                                        <div class="modal fade" id="myModal<?php echo $veiculo['cd_placa']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                                            <div class="modal-dialog" role="document">
                                                                <div class="modal-content">
                                                                    <div class="modal-header">
                                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <?php
        if ($multas->rowCount() == 0) {
            echo 'Não há multas para esse veiculo';
        }
        ?>
                                                                        <?php while ($multa = $multas->fetch(PDO::FETCH_ASSOC)) { ?> 
                                                                            <h4 class="modal-title text-center" id="myModalLabel">AIT Nº: <?php echo $multa['cd_ait']; ?></h4>
                                                                        </div>
    ...
    <?php
        $cd_placaProcesso = $veiculo['cd_placa'];

        try {
            $processos = $conexao->prepare("SELECT * FROM processo where cd_placa = :cd_placaProcesso");

            $processos->bindValue(":cd_placaProcesso", $cd_placaProcesso, PDO::PARAM_STR);
        ...
        ?>
                                                        ...
                                                        <div class="modal fade" id="myModalProcesso<?php echo $veiculo['cd_placa']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabelProcesso">
                                                            <div class="modal-dialog" role="document">
                                                                <div class="modal-content">
                                                                    <div class="modal-header">
                                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        ...
        if ($processos->rowCount() == 0) {
            echo 'Não há processos para esse veiculo';
        }
        ?>
                                                                        <?php while ($processo = $processos->fetch(PDO::FETCH_ASSOC)) { ?> 
                                                                            <h4 class="modal-title text-center" id="myModalLabelProcesso">Nº do Processo: <?php echo $processo['cd_processo']; ?></h4>
                                                                        </div>
                                                                        <div class="modal-body">
                                                                            <p>Ano do Processo: <?php echo $processo['aa_processo']; ?></p>
                                                                            <p>Resultado do processo: <?php if ($processo['ds_resultado'] == '') {
                                                                    echo 'Sem resultado';
                                                                } else {
                                                                    echo $processo['ds_resultado'];
                                                                } ?></p>
                                                                        <?php } ?>   </div> 
                                                                </div>
                                                            </div>
