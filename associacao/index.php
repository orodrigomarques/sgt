...
try {
    $contador = $conexao->prepare("SELECT count(*) as qtd FROM associacao "
            . "WHERE nm_razao_social LIKE  :associacao AND nm_linha LIKE :nm_linha  
                                  " . " " . $limit);

    $contador->bindValue(":associacao", '%' . $associacao . '%');

    $contador->bindValue(":nm_linha", '%' . $nm_linha . '%');
...
//End PAGINAÇÃO
try {
    $assoaciacoes = $conexao->prepare("SELECT * FROM associacao "
            . "WHERE nm_razao_social LIKE  :associacao AND  nm_linha LIKE :nm_linha
             " . "ORDER BY nm_razao_social ASC " . $limit);
    $assoaciacoes->bindValue(":associacao", '%' . $associacao . '%');

    $assoaciacoes->bindValue(":nm_linha", '%' . $nm_linha . '%');

    $assoaciacoes->execute();
} catch (Exception $e) {
    echo $e;
    exit();
}
                                        ...
                                        <div id="example_wrapper" class="dataTables_wrapper" role="grid">
                                            <form name="associacoes" method="POST" id="associacoes">
                                                <div class="row">
                                                    ...
                                                    <div class="col-xs-5">
                                                        <select name="nm_linha" id="nm_linha" class="form-control">
                                                            <option value='' >Linha...</option>
                                                            <?php
                                                            try {
                                                                $linhas = $conexao->prepare("SELECT * FROM associacao");
                                                            ...
                                                            ?>
                                                            <?php while ($linha = $linhas->fetch(PDO::FETCH_ASSOC)) { ?>
                                                                <option value='<?php echo $linha['nm_linha']; ?>'><?php echo $linha['nm_linha']; ?></option>
<?php } ?>                                              
                                                        </select> </div>
...
<?php if ($assoaciacoes->rowCount() > 0) { ?>
    ...
    <?php while ($associacao = $assoaciacoes->fetch(PDO::FETCH_ASSOC)) { ?>
                                                            <tr class="gradeA odd">
                                                                ...
                                                                <td style="width:40%" class="center">
                                                                    ...
                                                                    <a onClick="if (confirm('Tem certeza que deseja excluir este registro?')) {
                                                                                        location.href = 'gerencia.php?acao=excluir&id=<?php echo base64_encode($associacao['cd_associacao']); ?>'
