...
if (isset($_GET['acao']) && $_GET['acao'] != '') {
    ...
    if ($acao == 'excluir') {
        ...
        $deleta = mysqli_query($conexao, "DELETE FROM processo WHERE id_processo = '$id'");
    ...
    if ($acao == 'visualizar' || $acao == 'editar') {
        ...
        $processo = mysqli_fetch_assoc($linha) or die(mysql_error());
...
if (isset($_POST['id_processo']) && $_POST['id_processo'] != '') {
    ...
    if (empty($id)) {
        ...
        $sqlProcesso = "INSERT INTO processo (nm_modalidade, cd_processo, aa_processo, dt_relato_denuncia, dt_apresentacao_defesa,"
        ...
        echo $sqlProcesso;
        $res = mysqli_query($conexao, $sqlProcesso) or die(mysqli_error($conexao));
        $retorno = 'inserido';
    } else {
        $sqlProcesso = "UPDATE processo SET nm_modalidade = '" . $tipoVeiculo . "', cd_processo = " . $codigoProcesso . ", aa_processo = '" . $anoProcesso . "', dt_relato_denuncia = '" . $dataDenuncia . "', dt_apresentacao_defesa = '" . $dataDefesa . "', dt_apresentacao_relatorio = '" . $dataRelatorio . "', dt_inicio_julgamento = '" . $dataJulgamento . "', dt_julgado = '" . $dataJulgado . "', ds_resultado = '" . $resultado . "', dt_notificacao = '" . $notificacao . "', ds_observacoes_processos = '" . $observacoes . "' "
                . "WHERE id_processo = " . $id;

        $res = mysqli_query($conexao, $sqlProcesso) or die(mysqli_error());
        $retorno = 'alterado';
    }
