<?php 
   ...
   // $associacao = '';
    ...
    if(isset($_POST['nm_pessoa']) && !empty($_POST['nm_pessoa'])){
            $tipoPessoa = addslashes($_POST['nm_pessoa']);
            $tipoPessoa = str_replace('\\','',$tipoPessoa);
    }
    $where .= " WHERE nm_pessoa LIKE '%$tipoPessoa%'";
    ...
    $sql_tipopessoas =     "SELECT * FROM tipoPessoa ".
                        $where.			
			" ORDER BY nm_pessoa ASC ".
			$limit;   
    
    $tipopessoas = mysqli_query($conexao, $sql_tipopessoas);   
...
<script type="text/javascript" src="http://code.jquery.com/jquery-1.8.3.min.js"></script>
    ...
    $("td").dblclick(function() {
        ...
        $(this).html("<input type='text' name='usuario' id='usuario' value='" + conteudoOriginal + "' />");
        var usuario = $("#usuario");
                        ...
                        <div class="panel panel-primary">
                            ...
                            <div class="panel-body collapse in">
                                <div id="example_wrapper" class="dataTables_wrapper" role="grid">
                                    ...
                                    <?php if(mysqli_num_rows($tipopessoas) > 0){ ?>
                                            <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered datatables dataTable" id="example" aria-describedby="example_info">
                                                ...
                                                <?php while($tipopessoa = mysqli_fetch_assoc($tipopessoas)){?>
                                                        <tr class="gradeA odd">
                                                                <td style="width:5%" class="" id="teste" ><?php echo($tipopessoa['nm_pessoa']);?>&nbsp;&nbsp;&nbsp;&nbsp;</td>
...
<!--                                                                    <a onClick="buscaUsuario('<?php echo($tipopessoa['cd_tipo_pessoa']);?>');location.href='gerencia.php?acao=visualizar&id_usuario=<?php echo $usuario['id_usuario']; ?>'" class="btn btn-success"><i class="icon-trash">&nbsp;&nbsp;Visualizar</i> </a>-->
                                                                    <a onClick="if(confirm('Tem certeza que deseja excluir este registro?')){location.href='gerencia.php?acao=excluir&id=<?php echo base64_encode($tipopessoa['cd_tipo_pessoa']); ?>'}" class="btn btn-danger"> <i class="icon-trash">&nbsp;&nbsp;Excluir</i> </a>
                                                                </td>
                                                        </tr>
                                                <?php }?>
                                                </tbody>
